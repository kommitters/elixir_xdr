defmodule XDR.UIntErrorTest do
  use ExUnit.Case

  alias XDR.UIntError

  test "When receives :not_integer" do
    assert_raise UIntError, "The value which you try to encode is not an integer", fn ->
      raise UIntError, :not_integer
    end
  end

  test "When receives :not_binary" do
    assert_raise UIntError,
                 "The value which you try to decode must be a binary value, for example: <<0, 0, 0, 2>>",
                 fn ->
                   raise UIntError, :not_binary
                 end
  end

  test "When receives :exceed_upper_limit" do
    assert_raise UIntError,
                 "The integer which you try to encode exceed the upper limit of an unsigned integer, the value must be less than 4_294_967_295",
                 fn ->
                   raise UIntError, :exceed_upper_limit
                 end
  end

  test "When receives :exceed_lower_limit" do
    assert_raise UIntError,
                 "The integer which you try to encode exceed the lower limit of an unsigned integer, the value must be more than 0",
                 fn ->
                   raise UIntError, :exceed_lower_limit
                 end
  end
end
